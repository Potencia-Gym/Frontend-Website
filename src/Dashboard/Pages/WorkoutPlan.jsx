...
const WorkoutPlan = () => {
  ...
  //fetch recommended data from db only (no rerun of ML model)
  ...
  useEffect(() => {
    // to get recommended data (show form or recommended data? give if else condn here!!)
  ...
  const responseFunction = useCallback(
    (res, status) => {
      if (status === 200) {
  ...
  return (
    <div className="py-6 px-16 w-full h-full max-sm:px-8 max-md:px-10">
      {showForm ? (
        <form
          onSubmit={handleFormSubmit}
          className="max-w-[1100px] mt-3 flex flex-col gap-4 justify-center flex-1"
        >
          <div className="place-self-start">
            <h1 className="text-3xl max-sm:text-2xl mb-2 font-extrabold">
              Let&apos;s personalize your fitness plan
            </h1>
            <h3 className="text-green mb-3">
              We&apos;ll use these details to help you get the most out of your
              Potencia experience.
            </h3>
          </div>
          ...
          <div className="flex gap-5 flex-1 flex-col w-full justify-center">
            <div className="flex flex-col min-w-[280px] max-sm:w-full items-start justify-center">
              <label className="text-gray-200 max-xl:min-w-[97px] max-[360px]:min-w-[70px]">
                Height (cm)
              </label>
              <input
                name="height"
          ...
          <div className="min-w-[290px] max-sm:w-full flex flex-col gap-2 justify-center items-start">
            <p className="font-bold text-lg">
              Which body parts would you like to focus more on?
            </p>
            <Checkbox.Group
              rootClassName="gap-4 flex-wrap"
          ...
          <div className="w-full flex justify-end">
            <button
              type="submit"
              className="px-2 mb-3 bg-green hover:bg-green/70 focus:bg-indigo-400 text-white font-semibold rounded-xl py-2.5 mt-4"
            >
              Continue
            </button>
          </div>
      ...
      {!showForm ? (
        <div>
          <div className="ml-auto w-fit relative bottom-2">
            <button
              onClick={handleRefreshButton}
              className="bg-black p-3 rounded-full text-lg"
            >
              <FaSyncAlt />
            </button>
          </div>
          <div className="flex flex-wrap flex-1 gap-10 justify-center items-center mx-auto p-10 max-sm:p-2 max-w-[60vw] max-sm:max-w-[95vw] max-h-[90vh] bg-black rounded-xl overflow-y-scroll scrollbar border-y-[20px] border-black max-sm:border-y-[20px]">
              ...
              {days.map((val, idx) => (
                <div
                  ...
                  className={`flex group ${
                    val == selectedDay ? "bg-green" : "hover:bg-green"
                  } rounded-lg mx-1 transition-all duration-300	cursor-pointer justify-center w-16`}
                >
                  <div className="flex items-center px-4 py-4">
                    <div className="text-center">
                      <p className="text-white group-hover:text-gray-100 text-sm transition-all	duration-300">
                        {" "}
                        {val.charAt(0).toUpperCase() + val.substring(1, 3)}{" "}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {!loading
              ? recommendedData[selectedDay]?.map((val, key) => (
                  <ExerciseCard
                    exerciseData={val}
            ...
            {loading ? (
              <div role="status" className="flex justify-center items-center">
                <svg
                  aria-hidden="true"
                  className="inline w-12 h-12 text-gray-200 animate-spin dark:text-gray-600 fill-green"
                  viewBox="0 0 100 101"
                  fill="none"
                ...
                <span className="text-4xl px-3 dark:text-white tracking-wide">
                  Loading...
                </span>
              </div>
            ) : (
